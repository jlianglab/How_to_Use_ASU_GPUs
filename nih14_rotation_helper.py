...
parser.add_argument('--run', type=str, default="3_angle_rotation", help='multiple runs')
...
def test(test_loader, model,conf,best_file_path):
    """one epoch training"""
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(test_loader):
            ...
            tp1 = pred.argmax(dim=1)
            order_matches += (tp1 == target).sum()
            order_total += target.shape[0]

            if conf.debug_mode:
                break

    order_matches = order_matches.item()
    order_accuracy = order_matches / order_total

    print(">>Mean ACC = {:.4f}".format(order_accuracy), file=conf.log_writter)
...
def main(conf):
    ...
    train_dataset = ChestX_ray14_rotation(conf.data_root, conf.train_list, build_rotation_transform())
    valid_dataset = ChestX_ray14_rotation(conf.data_root, conf.valid_list, build_rotation_transform(mode="validation"))
    test_dataset = ChestX_ray14_rotation(conf.data_root, conf.test_list, build_rotation_transform(mode="test"))
    ...
    test(test_loader,model,conf, best_file_path)
    conf.log_writter.flush()
...
if __name__ == '__main__':
    ...
    #     test_dataset = ChestX_ray14_rotation(conf.data_root, conf.test_list, build_transform(mode="test_no_ten"))
    #     sampler_test = torch.utils.data.SequentialSampler(test_dataset)
    #     test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=conf.batch_size, num_workers=conf.num_workers, pin_memory=True, sampler=sampler_test)
